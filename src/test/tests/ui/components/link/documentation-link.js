...
module( "ui/components/link/documentation-link", function( hooks ) {
	...
	test( "Livestreamer documentation", async function( assert ) {
		this.providerType = "livestreamer";
		...
		assert.ok(
			this.openBrowserSpy.calledOnceWithExactly(
				"http://docs.livestreamer.io/cli.html#cmdoption--foo"
			),
			"Opens correct documentation URL in web browser"
		);
	});
	...
	test( "Custom baseUrl", async function( assert ) {
		await render( hbs`<DocumentationLink @item="--baz" @baseUrl="https://foo.bar/{item}" />` );
